...
let
  ...
  backups = [
    {
      name = "postgres";
      dataset = "threetbpool/postgres";
      mount = "${mountRoot}/postgres";
    }
    {
      name = "immich";
      dataset = "threetbpool/subvol-113-disk-0";
      mount = "${mountRoot}/immich";
    }
    {
      name = "opencloud";
      dataset = "threetbpool/subvol-101-disk-0";
      mount = "${mountRoot}/opencloud";
    }
  ];
  ...
  backupScript = pkgs.writeShellScriptBin "pve-zfs-backup" ''
    ...
    cleanup() {
      ...
      for item in ${
        builtins.concatStringsSep " " (map (b: "'${b.name}'") (pkgs.lib.reverseList backups))
      }; do
